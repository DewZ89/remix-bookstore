...
export const action = async ({ request }: ActionArgs) => {
  ...
  errors.password = password.length < 8 ? 'Password must contains at least 8 characters' : null
  
  errors.username = !username ? 'Username is required' : null
  errors.username = username.length < 3 ? 'Username must contains at least 3 characters' : null
  
  if (await getUserByEmail(email)) errors.email = "A user already exists with this email"
  if (await getUserByUsername(username)) errors.username = "A user already exists with this username"
  
  if (Object.values(errors).some((value) => value !== null))
    return json<ActionData>({
      errors, values: {email, username}
    })
...
export default function Join() {
  ...
  const {errors, values} = useActionData<ActionData>() || {};
      ...
      <div className="border-1 mx-auto w-full max-w-md rounded-md border-gray-500 bg-slate-100 px-10 py-8 opacity-90 shadow">
        <Form method="post" className="space-y-6">
          <div>
            ...
            <div className="mt-1">
              ...
              {Boolean(errors?.email) && (
                <div className="pt-1 text-red-700" id="email-error" role='alert'>
                  {errors?.email}
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div className='mt-1'>
              ...
              {Boolean(errors?.username) && (
                <div className='pt-1 text-red-700' id='username-error' role='alert'>
                  {errors?.username}
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div className="mt-1">
              ...
              {Boolean(errors?.password) && (
                <div className="pt-1 text-red-700" id="password-error" role='alert'>
                  {errors?.password}
                </div>
              )}
            </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-600 focus:bg-blue-400 disabled:bg-blue-200 active:bg-blue-700"
          >
